	...
	public static List<Usuario> todosUsuarios() {
		try{
	        ...
	        List lista = Arrays.asList(
	            ...
	            new Usuario("VALESKA LACERDA MELQUIADES", "03291078446", sdf.parse("1992/10/28"), Arrays.asList(Role.USER)), 
	            new Usuario("VALESSA DOS SANTOS MONTEIROS", "08267025464", sdf.parse("1992/09/18"), Arrays.asList(Role.USER)), 
	            new Usuario("LUCELIA LIMA FEITOSA", "04586068477", sdf.parse("1999/09/08"), Arrays.asList(Role.USER, Role.COLLABORATOR)), 
	            new Usuario("BENJAMIM BRITO MELO NETO", "05366152410", sdf.parse("2001/07/19"), Arrays.asList(Role.USER, Role.COLLABORATOR)), 
	            new Usuario("JOAO PEDRO HENRIQUE SANTOS DUARTE", "10944538460", sdf.parse("2001/06/17"), Arrays.asList(Role.USER, Role.MODERATE, Role.COLLABORATOR, Role.ADMIN)), 
